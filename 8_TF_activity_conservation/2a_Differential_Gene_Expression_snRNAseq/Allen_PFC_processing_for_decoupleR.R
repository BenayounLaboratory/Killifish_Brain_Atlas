setwd('/Volumes/BB_HQ_3/Brain_aging_Simons/scRNAseq/snRNAseq_Brain_Aging_Meta/Downstream_Analyses/Mouse_Datasets_for_Comparison/Muscat_DESeq2/Allen_PFC')
...
load('/Volumes/BB_HQ_3/Brain_aging_Simons/scRNAseq/snRNAseq_Brain_Aging_Meta/Downstream_Analyses/Mouse_Datasets_for_Comparison/Processed_Objects/Allen_GSE207848/2024-03-26_Allen_PFC_Brain_SingleCellNet_Annotated_Seurat_object.RData')
allen.singlets

table(allen.singlets@meta.data$SampleID,allen.singlets@meta.data$SingleCellNet_Hajdarovic)
...
allen.singlets.sce <- as.SingleCellExperiment(allen.singlets)
save(allen.singlets.sce, file = paste(Sys.Date(),"Allen_GSE207848_SingleCellExperimnent_object.RData",sep = "_"))
###############################################################################################
...
######################################################
####### Data preparation   ++++   Allen  PFC   #######
######################################################

allen.singlets.sce.cl <- prepSCE(allen.singlets.sce, 
...
nk  <- length(kids <- levels(allen.singlets.sce.cl$cluster_id))
ns  <- length(sids <- levels(allen.singlets.sce.cl$sample_id))
names(kids) <- kids; names(sids) <- sids

# Aggregation of single-cell to pseudobulk data
pb <- aggregateData(allen.singlets.sce.cl, assay = "counts", fun = "sum", by = c("cluster_id", "sample_id"))
...
cell.per.samp.tab <- t(table(allen.singlets.sce.cl$cluster_id, allen.singlets.sce.cl$sample_id))
cell.per.samp.tab
...
celltype.qc
# [1] "Astrocyte"                     "Microglia_Macrophage"          "Neuron"                        "Oligodendrocyte"              
# [5] "OPC"                           "Pericyte_EndothelialCell"      "VascularandLeptomeningealCell"

# extract pseudobulk information for samples that pass the cell number cutoff
counts.pb <- counts.pb.tmp[celltype.qc]

#### save counts
save(counts.pb, file = paste0(Sys.Date(),"_Allen_GSE207848_muscat_PB_objects_QC_Clean.RData"))
#############################################################################################
...
## load('2024-03-27_Allen_GSE207848_muscat_PB_objects_QC_Clean.RData')
...
my.meta$Age_month <- c(rep(1,4),rep(21,4))
my.meta$Sex       <- rep("F",8)
my.meta$SampleID  <- c("Y1", "Y2", "Y3", "Y4", "O1", "O2", "O3", "O4")
...
for  (i in 1:length(counts.pb)) {
  ...
  # get outprefix
  my.outprefix <- paste0(Sys.Date(),"_DEseq2_Pseudobulk_Allen_GSE207848_",names(counts.pb)[[i]])
...
#######################
sink(file = paste0(Sys.Date(),"_Allen_GSE207848_processing_session_Info.txt"))
sessionInfo()
sink()

setwd('/Volumes/BB_HQ_3/Brain_aging_Simons/scRNAseq/snRNAseq_Brain_Aging_Meta/Downstream_Analyses/Species_Comparison/Mouse_Datasets_for_Comparison/BulkRNA/TF_Activity_analysis')
...
# 2025-12-31
# add Hahn brain region bulk dataset

#####################################################################################################################
#### 0. Load up annotated DEseq2/VST objects

# Load necessary objects
load('../DESeq2/2025-08-08_Stilling_Hippocampus_DEseq2_objects.RData')
stilling.deseq.res.list  <- deseq.res.list ; rm(deseq.res.list)

load('../DESeq2/2025-08-08_Benayoun_Brain_DEseq2_objects.RData')
benayoun.deseq.res.list  <- deseq.res.list ; rm(deseq.res.list)

load('../DESeq2/2025-12-31_Hahn_Brain_DEseq2_objects.RData')
hahn.deseq.res.list  <- deseq.res.list ; rm(deseq.res.list)

ls()
# "benayoun.deseq.res.list" "hahn.deseq.res.list" "stilling.deseq.res.list"
#####################################################################################################################
...
Top_TFs_res.list        <- vector(mode = "list", length = 3)
names(Top_TFs_res.list) <- c("Benayoun", "Hahn", "Stilling")

my.deseq2.list <- list(benayoun.deseq.res.list, hahn.deseq.res.list, stilling.deseq.res.list)
names(my.deseq2.list) <- c("Benayoun", "Hahn", "Stilling")
...
for (k in 1:length(my.deseq2.list)) {
  ...
  colnames(Top_TFs_res.list[[k]]) <- c("statistic", "source","condition","score","p_value","Reg_Rank","Region")
  ...
  for (i in 1:length(my.deseq2.list[[k]])) {
    ...
    tf.sig$Reg_Rank   <- NA
    tf.sig$Region  <- names(my.deseq2.list[[k]])[i]
  ...
  my.num.comb   <- aggregate(Top_TFs_res.list[[k]]$Region, by = list("TF_regulon" = Top_TFs_res.list[[k]]$source), FUN = length)
  ...
  boxplot( my.sum.comb$N_CellType, col = "purple", main = "DecoupleR Regulon Sharing (QC cell types)", ylab = "Number of cell types", outline = F, ylim = c(0,15))
...
save(Top_TFs_res.list, file = paste0(Sys.Date(),"_decoupleR_fgsea_TFregulons_by_Region_MouseBrain_FDR10.RData"))

options(java.parameters = "-Xmx16g" )
require(openxlsx)

write.xlsx(Top_TFs_res.list, rowNames = F, file = paste0(Sys.Date(),"_Mouse_Brain_Aging_REGIONS_DecoupleR_Results_FDR10.xlsx"))
########################################################################################################################
...
for (i in 1:length(Top_TFs_res.list)) {
  ...
  # output data
  my.cell.types              <- sort(unique(Top_TFs_res.list[[i]]$Region))
    ...
    if(nrow(my.cur.tf) >0) {
      ...
      for (k in 1:nrow(my.cur.tf)) {
        ...
        if ( (my.cur.tf[k,]$sign > 0) && (my.cur.tf[k,]$p_value < 0.05)) {
          my.plot.table.mm[j,my.cur.tf[k,]$Region] <- 1
        } else if ( (my.cur.tf[k,]$sign > 0) && (my.cur.tf[k,]$p_value < 0.1)) {
          my.plot.table.mm[j,my.cur.tf[k,]$Region] <- 0.5
  ...
  pdf(paste0(Sys.Date(),"_KilliRecurrent_decoupleR_Regulons_byRegion_",names(Top_TFs_res.list)[i],"_FDR10.pdf"), width = 6, height = 8)
...
my.plot <- plot.list[[1]] + plot.list[[2]]  + plot.list[[3]]

pdf(paste0(Sys.Date(),"_KilliRecurrent_decoupleR_Regulons_byRegion_Mouse_Brain_ALL_DATA_FDR10_color.pdf"), width = 20, height = 8)
print(my.plot)
dev.off()
#####################################################################################################################

...
# 2025-12-31
# Parse/annotate Hahn 2025 Brain data for DecoupleR
# 13 brain regions

###############################################################################################
# 1. Read and preprocess data

my.cts.files <- c("../FASTQ/RNA_regions/Hahn/STAR/2025-08-15_Hahn_Caudate_putamen_Aging_counts_MM39.txt",
                  "../FASTQ/RNA_regions/Hahn/STAR/2025-08-15_Hahn_Cerebellum_Aging_counts_MM39.txt",
                  "../FASTQ/RNA_regions/Hahn/STAR/2025-12-29_Hahn_Choroid_Plexus_Aging_counts_MM39.txt",
                  "../FASTQ/RNA_regions/Hahn/STAR/2025-08-15_Hahn_Corpus_callosum_Aging_counts_MM39.txt",
                  "../FASTQ/RNA_regions/Hahn/STAR/2025-08-15_Hahn_Hippocampus_posterior_Aging_counts_MM39.txt",
                  "../FASTQ/RNA_regions/Hahn/STAR/2025-08-15_Hahn_Hypothalamus_Aging_counts_MM39.txt",
                  "../FASTQ/RNA_regions/Hahn/STAR/2025-08-15_Hahn_Medulla_Aging_counts_MM39.txt",
                  "../FASTQ/RNA_regions/Hahn/STAR/2025-08-15_Hahn_Motor_cortex_Aging_counts_MM39.txt",
                  "../FASTQ/RNA_regions/Hahn/STAR/2025-08-15_Hahn_Olfactory_bulb_Aging_counts_MM39.txt",
                  "../FASTQ/RNA_regions/Hahn/STAR/2025-08-15_Hahn_Pons_Aging_counts_MM39.txt",
                  "../FASTQ/RNA_regions/Hahn/STAR/2025-08-15_Hahn_Subventricular_zone_Aging_counts_MM39.txt",
                  "../FASTQ/RNA_regions/Hahn/STAR/2025-08-15_Hahn_Thalamus_Aging_counts_MM39.txt",
                  "../FASTQ/RNA_regions/Hahn/STAR/2025-08-15_Hahn_Visual_Cortex_Aging_counts_MM39.txt")

regions.names <- gsub("_Aging_counts_MM39.txt","", unlist(lapply(strsplit(my.cts.files,"_Hahn_"),'[[',2)))

my.cl.counts <- vector(mode = "list", length = length(my.cts.files))
names(my.cl.counts) <- regions.names
...
for (i in 1:length(my.cl.counts)) {
  ...
  colnames(my.cts)[-1] <- unlist(lapply(strsplit(unlist(lapply(strsplit(colnames(my.cts)[-1], "_RNA"),'[[',1)), "_C57BL6JN_"),'[[',2))
...
for ( i in 1:length(my.cl.counts)) {
  ...
  tmp.names         <- gsub(paste0(names(my.cl.counts)[i],"_"),"",colnames(my.cl.counts[[i]]))
  
  my.meta$Sex       <- factor(unlist(lapply(strsplit(tmp.names,"_"),'[[',1)), levels = c("F","M"))
  my.meta$Age_month <- as.numeric(unlist(lapply(strsplit(tmp.names,"_|mo",perl = T), '[[',2)))
  my.meta$Age_Group <- ifelse(my.meta$Age_month == 3, "Y", "O")
  my.meta$Age_Group <- factor(my.meta$Age_Group , levels = c("Y","O"))
  ...
  # get outprefix
  my.outprefix <- paste0(Sys.Date(),"_DEseq2_Hahn_", names(my.cl.counts)[i])
  ...
  # legend
  my.cols  <-  ifelse(my.meta$Sex == "M", 
                      ifelse(my.meta$Age_month == 3, "deepskyblue", "deepskyblue4"),
                      ifelse(my.meta$Age_month == 3, "deeppink", "deeppink4"))

  # get matrix using age as a modeling covariate
  dds <- DESeqDataSetFromMatrix(countData = sva.cts[[i]],
                                colData   = my.meta,
                                design    = ~ Age_month + Sex)
...
my.rdata.age <- paste0(Sys.Date(),"_Hahn_Brain_DEseq2_objects.RData")
save(deseq.res.list, file = my.rdata.age)

my.vst.age <- paste0(Sys.Date(),"_Hahn_Brain_VST_data_objects.RData")
save(vst.cts, file = my.vst.age)
##############################################################################################

#######################
sink(file = paste0(Sys.Date(),"_Hahn_Brain_processing_session_Info.txt"))
sessionInfo()
sink()
